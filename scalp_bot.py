...
# ========== UNIVERSE (top 20 by 24h volume – fast scan) ==========
STATIC_COINS = [
...
def get_live_price(symbol_usdt):
    """Fetch near-live price from CoinGecko. Returns 0 on failure."""
    coin_id = symbol_usdt.replace("USDT", "").lower()
    url = f"https://api.coingecko.com/api/v3/simple/price?ids={coin_id}&vs_currencies=usd"
    data = fetch_coingecko(url)
    if data and coin_id in data:
        return data[coin_id]["usd"]
    # fallback to yahoo last 5m close
    df = get_yahoo_klines(symbol_usdt, interval='5m', days=1)
    if not df.empty:
        return df['Close'].iloc[-1]
    return 0

# ========== LAYER 1: TECHNICALS (5m, structure‑heavy, no MACD) – weight 20% ==========
def get_technicals(symbol_usdt):
    ...
    # PRICE ACTION (structure, window=5 on 5m)
    window = 5
...
# ========== SIGNAL GENERATION (top 20 coins, live entry) ==========
def generate_signal():
    # Get current prices and top 20 coins by volume from CoinGecko
    ...
    for coin in candidates:
        sym = coin["symbol"]
        price = coin["price"]  # CoinGecko price (already live-ish)
        volume = coin["volume"]
    ...
    if best is None or abs(best_score) < 1.19:
    ...
    # --- Fetch live price for the best coin ---
    live_entry = get_live_price(best["symbol"])
    if live_entry <= 0:
        return {"action": "HOLD", "reasoning": "Could not fetch live price for entry."}

    # Recalculate stop and TPs using the live entry and the previously calculated ATR
    atr = best["atr"]
